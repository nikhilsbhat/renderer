...
BUILD_WITH_FLAGS="-s -w -X 'github.com/nikhilsbhat/renderer/version.Version=${VERSION}' -X 'github.com/nikhilsbhat/renderer/version.Env=${BUILD_ENVIRONMENT}' -X 'github.com/nikhilsbhat/renderer/version.BuildDate=${DATE}' -X 'github.com/nikhilsbhat/renderer/version.Revision=${REVISION}' -X 'github.com/nikhilsbhat/renderer/version.Platform=${PlATFORM}/${ARCHITECTURE}' -X 'github.com/nikhilsbhat/renderer/version.GoVersion=${GOVERSION}'"
...
publish: local.check local.build ## Builds and publishes the app
